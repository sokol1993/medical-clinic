...
var AuthenticationService = (function () {
    ...
    AuthenticationService.prototype.login = function (model) {
        ...
        return this.http.post(this.logUrl, model, options).map(function (response) {
    ...
    AuthenticationService.prototype.getRole = function (id) {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        var link = this.roleUrl + id;
        return this.http.get(link, options).map(function (response) {
            var user = response.json();
            if (user) {
